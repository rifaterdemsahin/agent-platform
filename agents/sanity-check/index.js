...
Scan the codebase and report any issues found.`;
  ...
  return { success: true, issues: parseIssues(result.content[0].text) };
}
...
async function runEnvValidate({ path, environment }, context) {
  ...
  try {
    ...
    return { success: true, valid: false, missing: [], warnings: [result.content[0].text] };
  }
}
...
async function runFullCheck({ path }, context) {
  const [lint, typecheck, env, config, deploy] = await Promise.all([
